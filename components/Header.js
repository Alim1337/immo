...
// CSS is moved to globals.css to avoid SSR/client hydration mismatch
// caused by template-literal quote escaping (&quot; vs ")
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, fontWeight: 300, letterSpacing: 5, color: GOLD, lineHeight: 1 }}>
            E-KRILI
          </div>
          <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: 7, letterSpacing: 4, color: FAINT, marginTop: 2 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        ...
        {loggedIn ? (
          <UserMenu name={proprietaireName} isVip={statusVIP} onDashboard={handleDashboard} onDisconnect={handleDisconnect} />
        ) : (
...
function SearchBar({ searchLocation, setSearchLocation, searchAdresse, setSearchAdresse, searchPropertyType, setSearchPropertyType, searchNumBedrooms, setSearchNumBedrooms, onSearch }) {
  const [focused, setFocused] = useState(false);
  return (
    <div
      style={{ display: 'flex', alignItems: 'stretch', height: 40, border: `1px solid ${focused ? BORDER_H : BORDER}`, background: focused ? '#F5F1EA' : 'rgba(237,233,225,0.6)', transition: 'border-color 0.25s' }}
    ...
    >
      {[
        ...
        <select key={i} value={value} onChange={e => setter(e.target.value)} className="lux-select"
          style={{ ...selectStyle, borderRight: last ? 'none' : `1px solid ${BORDER}` }}>
          {opts.map(o => <option key={o}>{o}</option>)}
        </select>
      ))}
      <button onClick={onSearch} className="lux-search-btn"
        style={{ background: GOLD, border: 'none', color: CREAM, fontFamily: "'Raleway', sans-serif", fontSize: 9, fontWeight: 500, letterSpacing: 3, padding: '0 22px', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'background 0.2s' }}>
        RECHERCHE
      </button>
    </div>
  );
...
function UserMenu({ name, isVip, onDashboard, onDisconnect }) {
  return (
    <Menu as="div" style={{ position: 'relative' }}>
      <Menu.Button
        style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'transparent', border: `1px solid ${BORDER}`, padding: '7px 14px', cursor: 'pointer', transition: 'border-color 0.2s' }}
        onMouseEnter={e => e.currentTarget.style.borderColor = BORDER_H}
        onMouseLeave={e => e.currentTarget.style.borderColor = BORDER}
      >
        {isVip && <FaCrown style={{ color: GOLD, fontSize: 11 }} />}
        <span style={{ fontFamily: "'Raleway', sans-serif", fontSize: 11, letterSpacing: 2, color: TEXT }}>{name}</span>
        <UserCircleIcon style={{ height: 16, color: FAINT }} />
        <Bars3Icon style={{ height: 16, color: FAINT }} />
      </Menu.Button>
      <Transition as={Fragment} enter="transition ease-out duration-120" enterFrom="transform opacity-0 translate-y-1" enterTo="transform opacity-100 translate-y-0" leave="transition ease-in duration-80" leaveFrom="transform opacity-100 translate-y-0" leaveTo="transform opacity-0 translate-y-1">
        <Menu.Items className="lux-menu-items" style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', width: 220, outline: 'none', padding: '8px 0' }}>
          {[
            ...
            <Menu.Item key={label}>
              {({ active }) => (
                <button onClick={action} className="lux-menu-item"
                  style={{ width: '100%', textAlign: 'left', background: active ? 'rgba(184,137,42,0.07)' : 'transparent', border: 'none', padding: '10px 18px', cursor: 'pointer', fontFamily: "'Raleway', sans-serif", fontSize: 10, letterSpacing: 2, color: active ? GOLD_L : MUTED, transition: 'all 0.15s' }}>
                  {label.toUpperCase()}
                </button>
              )}
            </Menu.Item>
          ...
          <Menu.Item>
            {({ active }) => (
              <button onClick={onDisconnect} className="lux-menu-item lux-menu-item-danger"
                style={{ width: '100%', textAlign: 'left', background: 'transparent', border: 'none', padding: '10px 18px', cursor: 'pointer', fontFamily: "'Raleway', sans-serif", fontSize: 10, letterSpacing: 2, color: active ? '#e07070' : 'rgba(220,100,100,0.7)', transition: 'color 0.15s' }}>
                DÉCONNEXION
              </button>
            )}
          </Menu.Item>
...
function GuestButtons({ router }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <button className="lux-outline-btn" onClick={() => router.push('/login')}
        style={{ background: 'transparent', fontFamily: "'Raleway', sans-serif", fontSize: 9, letterSpacing: 3, padding: '8px 18px', cursor: 'pointer' }}>
        CONNEXION
      </button>
      <button className="lux-gold-btn" onClick={() => router.push('/inscription')}
        style={{ fontFamily: "'Raleway', sans-serif", fontSize: 9, letterSpacing: 3, padding: '8px 18px', cursor: 'pointer', border: 'none' }}>
        CRÉER UN COMPTE
      </button>
    </div>
  );
